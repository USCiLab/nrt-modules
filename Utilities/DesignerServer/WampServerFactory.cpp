                            ...
                            void *user, void *in, size_t len)
{
  ...
  std::string msgtype;
    ...
    case LWS_CALLBACK_ESTABLISHED:
      ...
      pss->ws = new WampServer(context, wsi);
      ws = pss->ws;
...
WampServerFactory::WampServerFactory() :
itsRunning(false),
itsContext(nullptr)
{
...
void WampServerFactory::start(int port, std::string interface)
{
  ...
  itsRunning = true;
  itsThread = std::thread(std::bind(&WampServerFactory::serveRequests, this));
}

void WampServerFactory::stop()
{
...
void WampServerFactory::broadcastMessage(std::string message)
{
...
void WampServerFactory::registerProcedure(std::string const & messageName, std::function<std::string (rapidjson::Document const&)> callback)
{
  itsCallbacks[messageName] = callback;
}

void WampServerFactory::serveRequests()
{
  while(itsRunning)
