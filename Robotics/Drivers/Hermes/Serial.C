...
void Serial::start()
{
        ...
        printf("Looking Device Name [%s]",itsDevName.c_str());
        if(itsDevName != "")
        {
        ...
        write(cmd,1);
                ...
                if(itsCmdDevName == "")
                {
                        printf("Could Not Find Serial Device Matching Descriptor (%s)", itsDeviceDescription.c_str());
                }
        } else {
                printf("Opening from cmd line %s", itsCmdDevName.c_str());
...
void Serial::configureSearch(const std::string DeviceDescription, const int speed, const std::string SearchPrefix, const char *format,
                ...
                const int tout)
{
        printf("Search for %s in speed %d",DeviceDescription.c_str(),speed);
        itsDevName = "search";
...
int Serial::openPort(std::string DeviceName)
{
	...
	int fuser = 0;
        ...
        printf("Opening port %s", DeviceName.c_str());
...
std::vector<unsigned char> Serial::readFrame(const unsigned char frameStart, const unsigned char frameEnd, const int frameLength, const double timeout)
{
  ...
  if(frameLength > 0)
  {
    frameSize = frameLength;
		...
		//FIXXX:don't worry about timeout now
