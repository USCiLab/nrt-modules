...
// ######################################################################
ImageSinkModule::ImageSinkModule(std::string const & instanceid) :
    nrt::Module(instanceid),
    itsNameParameter(imagesink::NameDef, this),
    itsImageSink(new nrt::ImageSink("sink"))
{ 
  addSubComponent(itsImageSink);
}

// ######################################################################
void ImageSinkModule::onMessage(imagesink::Image msg)
{
  itsImageSink->out(msg->img, itsNameParameter.getVal());
}

// Don't forget this to be able to use your module as a runtime-loadable shared object
NRT_REGISTER_MODULE(ImageSinkModule);
